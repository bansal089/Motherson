...
page_selector = pd.date_range(start = '1/1/2009', end= '11/1/2023', freq= 'M')
dates= []
...
# col = ['Average temperature (°F)', 'Average humidity (%)','Average dewpoint (°F)', 'Average barometer (in)','Average windspeed (mph)', 
# 'Average gustspeed (mph)','Average direction (°deg)', 'Rainfall for month (in)','Rainfall for year (in)', 'Maximum rain per minute',
# 'Maximum temperature (°F)', 'Minimum temperature (°F)','Maximum humidity (%)', 'Minimum humidity (%)', 'Maximum pressure',
...
# formated_data = [df_list[i][1:] for i in range(len(df_list)) if len(df_list[i][1:]) == 19]

# final_data = pd.DataFrame(formated_data, columns = col, index = final_index)
# final_data.head()